...
class vaspwfc(object):
    ...
    def readWFHeader(self):
        ...
        # From VASP 5.x on, the Fermi energy is also written in this line, hence
        # change 12 to 13
    ...
    def wfc_r_Lauro(self, ispin=1, ikpt=1, iband=1,
                gvec=None, Cg=None, ngrid=None, phase=None,
                r0=[0.0, 0.0, 0.0]):
            '''
            Obtain the pseudo-wavefunction of the specified KS states in real space
            by performing FT transform on the reciprocal space planewave
...
#                            ),
#                            axis=1
#                        )).reshape(ngrid) # MAYBE GIVE THE FULL PHASE SET. This is actually quite optimized and fast. (8s of the overall 18min)
            ...
            gvec = self.gvectors(ikpt) # MAYBE GIVE THE FULL GVEC
            gvec %= ngrid[np.newaxis, :]

            phi_k = np.zeros(ngrid, dtype=np.complex128)

            phi_k[gvec[:, 0], gvec[:, 1], gvec[:, 2]
                ] = self.readBandCoeff(ispin, ikpt, iband) # MAYBE GIVE THE FULL CG SET

            phi_k = phi_k*normFac # Normalizing phi

            # perform complex2complex FFT
            return ifftn(phi_k) * phase
