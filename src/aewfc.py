...
class vasp_ae_wfc_Lauro(object):
    '''
    Refer to the following post for details.
    ...
    def __init__(
        ...
        # real space grid
        self._aegrid = np.asarray( aegrid, dtype=int )
        ...
        self._aecut = aecut
        self._aecut_cst = aecut

    def set_aecut(self, aecut=-4):
        '''
        '''
        # "aecut" is abs(aecut) multiples of 'pscut'
        self._aecut = np.abs(aecut) * self._pscut
    ...
    def get_beta_njk(self, Cg):
        '''
            Computing the PAW projector coefficients.
        '''
        self._beta_njk = self._q_proj.proj(Cg)

        return self._beta_njk

    def set_ae_ylm(self, epsilon=1e-10):
        '''
         Calculate the real spherical harmonics for a set of G-grid points up to
         LMAX.
        '''
        lmax = np.max([p.proj_l.max() for p in self._pawpp]) # PAW harmonics
        self.ylm = []
        for l in range(lmax+1): # loop over PAW harmonics
            ...
            xyz = np.asarray(self._ae_gk, dtype=float)
            ...
            last_m = sh_values.shape[1] - (2*l+1) # last m elements
            self.ylm.append( sh_values[:,last_m:] )
    ...
    def get_qijs(self): # Only used in get_ae_norm. Not in the main code
        '''
        Qij for all the atoms
    ...
    def get_nablaijs(self): # Only used in get_moment_mat. Not in the main code
        '''
        Matrix elements of the gradient operator for all the atoms
    ...
    def get_ae_wfc_Lauro(self,
        ...
        self.set_aecut(aecut) # Heavier part of the code. Band-independent

        Cg = self._pswfc.readBandCoeff(ispin, self._ikpt, iband, norm=False)
        beta_njk = self.get_beta_njk(Cg)

        # Loop to save some PAW and pseudo-wavefunction info
        nproj = 0
        terms_ae = []
        terms_ps = []
        for ii in range(self._natoms):
            itype = self._element_idx[ii]
            lmmax = self._pawpp[itype].lmmax
            beta_block = beta_njk[nproj:nproj + lmmax]
            iill = self.cqfak[itype]
            exp_iGR = self.crexp[:, ii]
            q_ae = self._q_ae_core[itype] * exp_iGR # (lmmax, n_G)
            q_ps = self._q_ps_core[itype] * exp_iGR # (lmmax, n_G)

            weighted = (iill * beta_block)  # shape (lmmax,)
            # Result shape: (n_G,)
            terms_ae.append((weighted @ q_ae))
            terms_ps.append((weighted @ q_ps))
            
            nproj += lmmax

        # Final accumulation
        Sg_ae = np.sum(terms_ae, axis=0)
        Sg_ps = np.sum(terms_ps, axis=0)
        ...
        # 3D FFT with three threads working in parallel
        return ifftn(phi_ae, workers=3) * fac
    
    def get_ae_wfc_k_Lauro(self,
            ikpt: int=1,
            ispin: int=1
        ):
        '''
            Computes the all-electron Kohn-Sham wavefunction for
            all bands and a fixed k-point in a vectorized manner.
        '''

        Nb = self._pswfc._nbands
        aecut = self._aecut_cst
        # the k-point vectors in fractional coordinates
        self._ikpt = ikpt
        self._kvec = self._pswfc._kvecs[ikpt - 1]
        ...
        self.set_aecut(aecut) # Band-independent part

        Cg = np.array([ self._pswfc.readBandCoeff(ispin=ispin, ikpt=self._ikpt, \
                                    iband=nu+1, norm=False) for nu in range(Nb) ])
        beta_njk = np.array([ self.get_beta_njk(Cg[nu]) for nu in range(Nb) ])
        
        # Loop to save some PAW and pseudo-wavefunction info
        nG = self.crexp.shape[0]
        nproj_total = beta_njk.shape[1]

        q_ae_full = np.zeros((nproj_total, nG), dtype=complex)
        q_ps_full = np.zeros((nproj_total, nG), dtype=complex)

        nproj = 0 # Number of PAW projectors
        for ii in range(self._natoms): # loop over atomic potentials (# atoms)
            itype = self._element_idx[ii] # indexing atomic elements
            lmmax = self._pawpp[itype].lmmax # PAW projectors of atomic type
            iill = self.cqfak[itype]
            exp_iGR = self.crexp[:, ii]  # shape (n_G,)
            
            # q_ae_core and q_ps_core shape: (lmmax, n_G)
            q_ae = self._q_ae_core[itype]
            q_ps = self._q_ps_core[itype]
            
            # Precompute phase-shifted and iill-weighted projectors
            # scaled_q = iill[:, None] * q * exp(iGR)
            q_ae_full[nproj:nproj+lmmax] = (iill[:, None] * q_ae) * exp_iGR[None, :]
            q_ps_full[nproj:nproj+lmmax] = (iill[:, None] * q_ps) * exp_iGR[None, :]
            
            nproj += lmmax

        Sg_ae = beta_njk @ q_ae_full   # shape: (Nb, nG)
        Sg_ps = beta_njk @ q_ps_full   # shape: (Nb, nG)

        shape_phi = (Nb, *self._aegrid)
        phi_ae = np.zeros(shape_phi, dtype=complex)

        # Reshaping to 1+3D (band dimension + real space)
        g1 = self._ps_gv % self._aegrid[None,:]
        g2 = self._ae_gv % self._aegrid[None,:]
        ...
        # 3D FFT with three threads working in parallel
        return np.array( [ ifftn(phi, workers=3) for phi in phi_ae ] ) * fac
